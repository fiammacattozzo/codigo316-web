/* ============================================================
   ...
   Columnas de la planilla (en este orden):
     id | nombre | precio (o valor) | icono | fotos | categoria | talles | activo
   ...
   - fotos: nombres de archivo en la carpeta imagenes/, separados
     por coma (ej: imagenes/producto-1.jpg,imagenes/producto-2.jpg)
   - categoria: camperas / remeras / pijamas / buzos / mochilas
     / cartucheras / otros — se usa para los filtros del catálogo
   - talles: OPCIONAL. Si el producto es de niño, dejar vacío (usa
     6,8,10,12,14,16 por default). Si es de adulto o necesita otro
     rango, escribir los talles separados por coma, ej: S,M,L,XL,XXL
...
const CATEGORIAS = [
  ...
  { valor: "mochilas",    etiqueta: "Mochilas" },
  { valor: "cartucheras", etiqueta: "Cartucheras" },
  { valor: "otros",       etiqueta: "Otros" },
];

// Categorías de ropa que piden talle por default. Mochilas, tazas, cartucheras, etc. no lo necesitan,
// salvo que el producto tenga su propia columna "talles" cargada en la planilla.
const CATEGORIAS_CON_TALLE = ["camperas", "remeras", "buzos", "pijamas"];
const TALLES_NINO_DEFAULT = ["6", "8", "10", "12", "14", "16"];
...
const PRODUCTOS_RESPALDO = [
  ...
  { id: "sprunky-mochila-escolar", nombre: "Sprunki — Mochila escolar", precio: 42000, icono: "🎒",
    fotos: ["imagenes/sprunky-mochila-escolar-1.jpg","imagenes/sprunky-mochila-escolar-2.jpg"], categoria: "mochilas" },
  { id: "miloj-buzo-capucha", nombre: "Mil.J — Buzo con capucha", precio: 34500, icono: "🤿",
    fotos: ["imagenes/miloj-buzo-capucha-1.jpg","imagenes/miloj-buzo-capucha-2.jpg","imagenes/miloj-buzo-capucha-3.jpg","imagenes/miloj-buzo-capucha-4.jpg"], categoria: "buzos" },
  { id: "diadelpadre-remera-varios", nombre: "Día del Padre — Remera (varios diseños)", precio: 22000, icono: "👕",
    fotos: ["imagenes/diadelpadre-remera-varios-1.jpg","imagenes/diadelpadre-remera-varios-2.jpg","imagenes/diadelpadre-remera-varios-3.jpg","imagenes/diadelpadre-remera-varios-4.jpg","imagenes/diadelpadre-remera-varios-5.jpg"],
    categoria: "remeras", talles: ["S","M","L","XL","XXL"] },
...
function listaTalles(p){
  if(p.talles && p.talles.length) return p.talles;
  if(CATEGORIAS_CON_TALLE.includes(p.categoria)) return TALLES_NINO_DEFAULT;
  return [];
}
function requiereTalle(p){
  return listaTalles(p).length > 0;
}
...
async function cargarProductosDesdeSheet(){
  try{
    ...
    const idxTalles = encabezado.indexOf("talles");
    ...
    const productosPlanilla = filas.slice(1).map(f => ({
      ...
      talles: idxTalles >= 0 ? (f[idxTalles] || "").split(",").map(t => t.trim()).filter(Boolean) : [],
...
function tarjetaHTML(p){
  ...
  const talles = listaTalles(p);
  if(usaTalle && !tallaSeleccionada[p.id]){
    tallaSeleccionada[p.id] = talles[0];
  }
...
function cambiarCantidad(id, delta){
  const p = PRODUCTOS.find(x => x.id === id);
  if(!p) return;
  const talla = requiereTalle(p) ? (tallaSeleccionada[id] || listaTalles(p)[0]) : null;
  const clave = claveCarrito(id, talla);
